...
const Signup: React.FC = () => {
  ...
  const handleGoogleSignup = async () => {
    if (!googleConfig?.clientId) {
      setErrors({ general: "Google authentication is not configured on this server." });
      return;
    }
    setErrors({});
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.error || json?.message || "Google signup failed");
      }
      ...
      setErrors({ general: err?.message || "Failed to continue with Google" });
    } finally {
          ...
          <Card className="bg-white border border-slate-200/90 shadow-xl shadow-slate-200/50 rounded-3xl overflow-hidden">
            <CardContent className="p-6 sm:p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Social Signup Options (Google & Facebook SSO) */}

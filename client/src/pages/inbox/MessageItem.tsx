      ...
      >
        ...
        {isOutbound && onReply && (
          <button
            ...
            className="p-1.5 opacity-0 group-hover:opacity-100 transition-opacity bg-white hover:bg-gray-50 rounded-full border shadow-sm mb-2 text-gray-500 hover:text-gray-700"
            title="Reply"
          >
        ...
        >
          ...
          {(message.metadata as any)?.replyTo && (
            <div className={cn(
              "mb-2 p-2 rounded-lg text-sm border-l-4",
              (message.metadata as any).replyTo.fromUser 
                ? "bg-green-50 border-green-500" 
                : "bg-gray-50 border-gray-400"
            )}>
              <span className={cn(
                "block text-xs font-semibold mb-1",
                (message.metadata as any).replyTo.fromUser ? "text-green-700" : "text-gray-700"
              )}>
                {(message.metadata as any).replyTo.fromUser ? "You" : "Contact"}
              </span>
              <span className="text-gray-600 truncate block opacity-80">
                {(message.metadata as any).replyTo.content || "Media message"}
              </span>
            </div>
          )}

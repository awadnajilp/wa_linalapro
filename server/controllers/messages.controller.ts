...
export const createMessage = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (replyToMessageId) {
    try {
      ...
      if (originalMessage && originalMessage.whatsappMessageId) {
        replyToWaId = originalMessage.whatsappMessageId;
        finalMetadata.replyTo = {
          id: originalMessage.id,
          whatsappMessageId: originalMessage.whatsappMessageId,
          content: originalMessage.content,
          fromUser: originalMessage.fromUser,
          messageType: originalMessage.messageType
        };
      }
    } catch (e) {
      console.error("Failed to resolve replyToMessageId:", e);
    }
  }
...
if (file.size > MAX_SIZE_MB * 1024 * 1024) {
  throw new Error(`❌ ${file.originalname} exceeds WhatsApp size limit (${MAX_SIZE_MB}MB).`);
      ...
      // PLAIN TEXT (blocked if 24-hour window expired)
        ...
        try {
          result = await whatsappApi.sendTextMessage(conversation.contactPhone, content, replyToWaId);
        } catch (err: any) {
